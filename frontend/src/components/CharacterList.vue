<template>
  <div class="bg-white/10 rounded-2xl h-full border border-white/15 flex flex-col overflow-hidden shadow-xl backdrop-blur">
    
    <div class="bg-gradient-to-r from-[#781D42]/80 to-[#A3423C]/80 p-4 border-b border-white/10 flex-shrink-0 flex justify-between items-center">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-white/70">{{ characters.length }} terdeteksi</p>
        </div>
      </div>
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar p-4 relative">
      ...
      <div v-if="characters.length === 0" class="absolute inset-0 flex flex-col items-center justify-center text-white/60 space-y-2">
      ...
      <TransitionGroup name="list" tag="div" class="space-y-3">
        <div 
          ...
          :key="char.name + index" 
          class="bg-white/5 p-3 rounded-xl border border-white/10 flex gap-3 items-center hover:bg-white/10 transition-colors group"
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-2 mt-2">
                <div class="flex-1 bg-white/10 h-1.5 rounded-full overflow-hidden">
                  <div 
                    class="h-full bg-gradient-to-r from-[#F0D290] via-[#A3423C] to-[#781D42]"
                    :style="{ width: char.confidence + '%' }"
                  ></div>
                </div>
                <span class="text-[10px] text-[#F0D290] font-bold">{{ char.confidence }}%</span>
            </div>
          </div>
        </div>
      </TransitionGroup>
